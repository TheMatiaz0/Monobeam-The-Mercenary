...
namespace TheMatiaz0_MonobeamTheMercenary.Serialization
{
    public static class SerializationJSON
    {
        ...
        public static SortedDictionary<int, string> ReadFromJsonFileList<T>(string filePath)
        {
            TextReader reader = null;
            try
            {
                reader = new StreamReader(filePath);
                string fileContents = reader.ReadToEnd();
                return JsonConvert.DeserializeObject<SortedDictionary<int, string>>(fileContents);
            }
